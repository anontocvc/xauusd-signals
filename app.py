...
from setup_page import get_setup_html
...
def setup_page():
    import os
    return Response(get_setup_html(
        src    = fetcher._source or "none",
        td_key = os.environ.get("TWELVE_DATA_KEY","").strip(),
        av_key = os.environ.get("ALPHA_VANTAGE_KEY","").strip(),
        tg_tok = os.environ.get("TELEGRAM_TOKEN","").strip(),
        tg_cid = os.environ.get("TELEGRAM_CHAT_ID","").strip(),
    ), mimetype="text/html")
